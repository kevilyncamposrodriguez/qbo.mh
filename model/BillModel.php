...
class InvoiceModel {
            ...
            while($invoicess =  $dataService->Query("select * from Invoice where TxnDate = '" . $fecha . "'", $i, 100)){
                ...
                foreach($invoicess as $invoice){
                   ...
                   if(file_exists("files/".$client[0]['idcard']."/Creados/SinEnviar/".$clave."/".$clave.".xml") && $invoice->CustomField[0]->StringValue == 'aceptado'){
                       ...
                       $this->sendFiles($mail, $correo, $clave, $client[0]['idcard'],$client[0]['name']);
    ...
    public function sendFiles($mail, $to, $clave, $idcard, $cliente){
         $mail->isSMTP(); 
...
'); 
    ...
    public function all($dataService) {
        $dt_Ayer = date('Y-m-d', strtotime('-6 day')); // resta 3 dÃ­a 
                    ...
                    if (substr($invoice["DocNumber"], 8, 2) == '01') {
                        if (substr($invoice["DocNumber"], 8, 2) == '01') {
                            ...
                            if($invoice["CustomerMemo"]==""){
                                ...
                                $result = $this->syncFEMH($dataService,$client,$result,$invoice["Id"],$clave);
                            }else{
                                $clave = str_replace("Clave: ", "", $invoice["CustomerMemo"]);
                                $result = $this->syncFEMHConsulta($dataService,$client,$clave,$invoice["Id"]);
                            }
                            $cont++;
    ...
    public function syncTE($clients) {
        try {
            ...
            $cont = 0;
            foreach ($invoices as $invoice) {
                echo "correo ".$invoice["BillEmail"]["Address"];
                if ($invoice['CustomField'][0]['StringValue'] == '') {
    ...
    public function syncFEMHConsulta($dataService,$client ,$clave, $invoiceId){
    ...
    public function syncFEMH($dataService, $client, $result, $invoiceId,$clave){
        ...
        if($envio["status"]=="202" || $envio["status"]=="200"){
            if($envio["ind-estado"]!= "" && $envio["ind-estado"]!= "procesando"){
                ...
                $this->upload($dataService,"Respuesta",$envio["message"],$invoiceId,$result["clave"]);
                echo "<br> upload respuesta en qb";
                $this->updateState($dataService,$invoiceId, $envio["ind-estado"],$client,$clave);
                echo "<br> upload estado en qb";
                echo "<br>Respuesta: ".$envio["ind-estado"];
                //envia xmls al correo
                if($envio["ind-estado"] == 'aceptado'){
                    //$this->sendMailXML($to,$clave,$idcard,$client);
                }
    ...
    public function updateState($dataService,$invoiceId, $state,$client,$clave)
    {
		...
		if($state == "aceptado" && $client["ea"]== "1" && substr($invoice ->DocNumber, 8, 2) == '01'){
		   ...
		   $mail = new PHPMailer; 
		   $clave = str_replace ( "Clave: " , "" , $invoice->CustomerMemo);
		   $this->sendFiles($mail, $invoice->BillEmail->Address, $clave,$client["idcard"], $client["name"]);
		}
		$error = $dataService->getLastError();
		if ($error) {
		    return false;
