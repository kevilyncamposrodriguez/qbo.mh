...
<ol class="breadcrumb float-xl-right">
    ...
    <li class="breadcrumb-item active"><a>Procesados</a></li>
</ol>
<!-- end breadcrumb -->
<!-- begin page-header -->
<h1 class="page-header">Listado de comprobantes Procesados<small></small></h1>
<!-- end page-header -->
<!-- begin panel -->
    ...
    <div class="col-xl-12">
        <div class="panel panel-inverse">
            ...
            <div class="panel-heading">
                <h4 class="panel-title">Tabla de comprobantes Procesados</h4>
               <form class="form-horizontal form-bordered" >
					<div class="form-group row">
						<label class="col-lg-2 col-form-label">Filtrar</label>
						<div class="col-lg-10">
                ...
                <table id="data-table-select" class="table table-striped table-bordered table-td-valign-middle">
                    <thead>
                        <tr>
                            ...
                            <th class="text-nowrap">Respuesta MH</th>
                            <th class="text-nowrap">Estado QBO</th>
                            <th class="text-nowrap">Total</th>
                            <th class="text-nowrap">Fecha Procesado</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        ...
                        foreach ($data as $value) {
                            ...
                            if (file_exists('files/' . $_SESSION['idCard'] . '/Recibidos/Procesados/' . $value['clave'] . '/' . $value['clave'].'.pdf')) {
                                echo '<a class="btn btn-default fa fa-search" href="files/' . $_SESSION['idCard'] . '/Recibidos/Procesados/' . $value['clave'] . '/' . $value['clave'] . '.pdf" target="_blank"> </a>';
                            } else {
                            ...
                            $filename = 'files/' . $_SESSION['idCard'] . '/Recibidos/Procesados/' . $value['clave'] . '/' . $value['clave'] . '.xml';
                            ...
                            $filenamer = 'files/' . $_SESSION['idCard'] . '/Recibidos/Procesados/' . $value['clave'] . '/' . $value['clave'] . '-R.xml';
                            if (file_exists($filenamer)) {
                                echo ' <a class="btn btn-default" download="' . $value['clave'] . '-R.xml" href="files/' . $_SESSION['idCard'] . '/Recibidos/Procesados/' . $value['clave'] . '/' . $value['clave'] . '-R.xml' . '" title = "Descargar XML Respuesta" <i class = " txt-color-blue">R</i></a>';
                            }
                            $filenamer = 'files/' . $_SESSION['idCard'] . '/Recibidos/Procesados/' . $value['clave'] . '/Confirmacion-' . $value['clave'] . '.pdf';
                            if (file_exists($filenamer)) {
                                echo ' <a class="btn btn-default" download="Confirmacion-' . $value['clave'] . '.pdf" href="files/' . $_SESSION['idCard'] . '/Recibidos/Procesados/' . $value['clave'] . '/Confirmacion-' . $value['clave'] . '.pdf' . '" title = "Descargar pdf Confirmacion" <i class = " txt-color-blue">C</i></a>';
                            }
                           // if( $value['RespuestaQBO'] == '' &&  $value['estado'] != 'Rechazado' ){
                                echo ' <a class="btn btn-default fa fa-save" title = "Cargar archivos a QB" href="?controller=Expenses&action=saveQBP&key=' . $value['clave'] . '"></a>';
                           // }
                            echo' </div>  </td>';
                            echo '<td>' . substr($value['fecha'][0], 0, 10) . '</td>';
                            echo '<td>' . "D" . $value['clave'] . '</td>';
                            echo '<td>' . $value['Emisor'][0] . '</td>';
                            echo '<td>' . $value['estado'] . '</td>';
                            echo '<td>' . $value['RespuestaMH'] . '</td>';
                            echo '<td>' . $value['RespuestaQBO'] . '</td>';
                            echo '<td>' . $value['monto'] . '</td>';
                            echo '<td>' . $value['fechaprocesado'] . '</td>';
                            echo '</tr>';
...
<script type="text/javascript">
    function viewInvoice(datos) {
        ...
        sumaOtros = 0;
        if(datos["others"]){
            for (var o in datos["others"]) {
                detalles += '<tr>' +
                    '<td>' + cont + '</td>' +
                    '<td>' + "Otro Cargo - " + datos["others"][o]["Detalle"] + '</td>' +
                    '<td class="right">1</td>' +
                    '<td class="right">' +datos["others"][o]["MontoCargo"] + '</td>';
                ...
                detalles +=        
                    '<td class="right">0</td>';  
                detalles +=
                    '<td class="right"><strong>' + datos["others"][o]["MontoCargo"] + '</strong></td>' +
                    '</tr>';
                    cont++;
            }
        }
        ...
        document.getElementById("subtotal").innerHTML = (datos["monto"] - datos["impuesto"]).toFixed(5);
            ...
            <div class="modal-body">
                ...
                <div class="invoice">
                    ...
                    <div class="invoice-note">
                        * Vista genereda por el sistema de sincronizacion, el archivo PDF no se a podido relacionar con el xml de la factura
                    </div>
                    <!-- end invoice-note -->
